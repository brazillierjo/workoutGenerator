...
export default function FilteredExos(props) {
    ...
    return <div>
        <div className='flex flex-wrap justify-around bg-gray-200 py-1 mx-8 rounded-md outline-none'>
            {group?.map(group => {
                return <button className={selectedMuscle === group ? 'bg-cyan-500 shadow px-6 py-2.5 text-sm leading-5 font-medium text-white rounded-md outline-none focus:outline-none focus:ring-2 ring-offset-2 ring-offset-blue-400 ring-white ring-opacity-60' : 'px-6 py-2 rounded-md'} key={group} onClick={() => setSelectedMuscle(group)}>{group}</button>
            })}
        </div>
        <br />
        {
                ...
                if (exo.id === selectedMuscle) {
                    return <div className='bg-gray-200 p-2 text-sm rounded-md my-3 mx-2' key={exo.name}>
                        <div className='flex'>
                            ...
                            <div className='w-4/6'>
                                <h2 className='text-lg mb-3 w-11/12'>{exo.name}</h2>
